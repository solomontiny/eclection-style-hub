...
function InventoryPage() {
  ...
  const rows = useMemo(() => {
    return data.filter((p) => {
      ...
      if (filter === "low" && !(p.stock > 0 && p.stock <= threshold)) {
        return false;
      }
  ...
  const stats = [
    ...
    { label: "Units in stock", value: totalUnits, tone: "" },
    {
      label: "Low stock",
      value: lowCount,
      tone: lowCount ? "text-amber-600" : "",
    },
    {
      label: "Out of stock",
      value: outCount,
      tone: outCount ? "text-red-600" : "",
    },
  ];
      ...
      <div className="flex items-end justify-between flex-wrap gap-3">
        ...
        {lowCount + outCount > 0 && (
          <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 px-3 py-2 rounded-lg">
            ...
            {lowCount + outCount > 1 ? "s" : ""} need attention
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          ...
          >
            ...
            <p className={`font-display text-2xl mt-2 ${s.tone}`}>
              {s.value}
            </p>
          </div>
        ))}
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Input
                            ...
                            onBlur={(e) => {
                              const v = Math.max(0, Number(e.target.value));
                              if (v !== p.stock) {
                                mutation.mutate({ id: p.id, stock: v });
                              }
                            }}
                          />
